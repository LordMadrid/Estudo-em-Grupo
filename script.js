...
function mostrarGrupo() {
  ...
  db.collection("respostas").get().then(snapshot => {
    ...
    const perguntas = Object.keys(respostasObj)
      .sort((a, b) => a.localeCompare(b, undefined, { numeric: true }))
      .map(key => respostasObj[key].pergunta);
    perguntas.forEach(p => html += `<th>${p}</th>`);
    html += `</tr></thead><tbody>`;

    snapshot.forEach(doc => {
      const { nome, respostas, sugestao } = doc.data();
      let pro = 0, contra = 0;
      html += `<tr><td>${nome}</td>`;

      Object.entries(respostas).forEach(([key, resp]) => {
        if (key === 'sugestao') return;
      ...
      if (sugestao) {
        html += `<tr><td colspan="${perguntas.length + 1}"><em>Sugestão de ${nome}:</em> ${sugestao}</td></tr>`;
      }
    });
